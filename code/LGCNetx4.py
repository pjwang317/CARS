...
class SR(object):
    def __init__(self):
        ...
        if mode == "test_WV3":
            self.train_dir = './checkpoint/DFC_LGCNet'
            self.save_path = './test_full/DFC_WV3_LGCNet/'
        elif mode == "test_pleiades":
            self.train_dir = './checkpoint/DFC_LGCNet_pleiades'
            # self.save_path = './test_full/DFC_pleiades_LGCNet/'
            self.save_path = './test/pleiades_LGCNet_new/'
        elif mode == 'train':
            self.train_dir = './checkpoint/DFC_LGCNet'
        ...
        # self.pathlist = open('./data/filelist_trainK.txt', 'rt').read().splitlines()
        # random.shuffle(self.pathlist)
        # self.vallist = open('./data/filelist_valK.txt', 'rt').read().splitlines()
        ...
        for i in range(num_frame):
            ...
            with tf.variable_scope('srmodel', reuse=reuse) as scope_sr:#prelu
                with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.relu, stride=1,
                    ...
                    conv1 = slim.conv2d(conv0, filters, [3, 3], scope='conv1')
                    conv2 = slim.conv2d(conv1, filters, [3, 3], scope='conv2')
                    conv3 = slim.conv2d(conv1, filters, [3, 3], scope='conv3')
                    conv4 = slim.conv2d(conv1, filters, [3, 3], scope='conv4')
                    conv5 = tf.concat([conv2, conv3, conv4], axis = 3)
                    conv6 = slim.conv2d(conv5, filters *2, [5, 5], scope='conv6')
                    conv7 = slim.conv2d(conv6, num_channels, [3, 3], scope='conv7')
