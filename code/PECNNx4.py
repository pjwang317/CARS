...
class SR(object):
    def __init__(self):
        ...
        if mode == "test_WV3":
            self.train_dir = './checkpoint/DFC_PECNNx4'
            # self.save_path = './test/DFC_WV3_PECNNx4/'
            self.save_path = './test_full/DFC_WV3_PECNNx4/'
        elif mode == "test_pleiades":
            self.train_dir = './checkpoint/PECNN_pleiadesx4'
            # self.save_path = './test/DFC_pleiades_PECNNx4/'
            self.save_path = './test/pleiades_PECNNx4_new/'
            # self.save_path = './test_full/DFC_pleiades_PECNNx4/'
        elif mode == 'train':
            self.train_dir = './checkpoint/DFC_PECNNx4'
        ...
        for i in range(num_frame):
            ...
            with tf.variable_scope('srmodel', reuse=reuse) as scope_sr:#prelu
                with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.relu, stride=1,
                    ...
                    conv2 = slim.conv2d(conv1, 48, [3, 3], scope='conv2')
                    #conv2 = slim.conv2d(conv1, 27, [3, 3], scope='conv2')
                    base_sr = modules.ps._PS(conv2, self.scale_factor, 3)
                    down_samples = modules.ps.down_sample(base_sr,self.scale_factor)
                    ...
                    conv10 = slim.conv2d(res_in1, 48, [3, 3], activation_fn= None, scope='conv10')
