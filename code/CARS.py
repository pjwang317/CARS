### this is a channel attention based remote sensing image super-resolution
...
class SR(object):
    def __init__(self):
        self.num_frames = 1
        ### num_repeat_RRDB is in 5, 8, 10, 15
        self.num_RRDB = 15
        self.crop_size = 64
        self.scale_factor = 4
        ...
        self.use_densely = True
        self.use_CA = True
        self.use_Transfer = False
        
        if self.use_Transfer:
            if dataset == 'pleiades':
                self.max_steps = int(3.3e5+1)
            elif dataset == 'WV3':
                self.max_steps = int(2.2e5+1)
        else:
            self.max_steps = int(1.1e5+1)
        self.batch_size = 4
        ...
        if self.use_CA:
            if self.use_Transfer:
                if dataset == 'pleiades':
                    self.train_dir = './checkpoint/{}_{}_{}_B{}_TT'.format(dataset, method, met, self.num_RRDB)
                    self.save_path = './test/{}_{}_{}_B{}_T_new'.format(dataset, method, met, self.num_RRDB)
                    # self.save_path = './testx4_full/{}_{}_{}_B{}_T'.format(dataset, method, met, self.num_RRDB)
                else:
                    self.train_dir = './checkpoint/{}_{}_{}_B{}_T'.format(dataset, method, met, self.num_RRDB)
                    self.save_path = './test/{}_{}_{}_B{}_T_new'.format(dataset, method, met, self.num_RRDB)
                    # self.save_path = './testx4_full/{}_{}_{}_B{}_T'.format(dataset, method, met, self.num_RRDB)
            else:
                if dataset == 'pleiades': 
                    self.train_dir = './checkpoint/{}_{}_{}_B{}_T'.format(dataset, method, met, self.num_RRDB)
                    self.save_path = './test/{}_{}_{}_B{}_new'.format(dataset, method, met, self.num_RRDB)
                    
                    # self.save_path = './testx4_full/{}_{}_{}_B{}'.format(dataset, method, met, self.num_RRDB)
                else:
                    self.train_dir = './checkpoint/{}_{}_{}_B{}'.format(dataset, method, met, self.num_RRDB)
                    self.save_path = './test/{}_{}_{}_B{}_new'.format(dataset, method, met, self.num_RRDB)
            ...
            if self.use_Transfer:
                self.train_dir = './checkpoint/{}_{}_{}_B{}_NCAT'.format(dataset, method, met, self.num_RRDB)
                self.save_path = './test/{}_{}_{}_B{}_NCAT_new'.format(dataset, method, met, self.num_RRDB)
                # self.save_path = './testx4_full/{}_{}_{}_B{}_NCAT'.format(dataset, method, met, self.num_RRDB)
            
            else:
                self.train_dir = './checkpoint/{}_{}_{}_B{}_NCA'.format(dataset, method, met, self.num_RRDB)
                self.save_path = './test/{}_{}_{}_B{}_NCA_new'.format(dataset, method, met, self.num_RRDB)
                # self.save_path = './testx4_full/{}_{}_{}_B{}_NCA'.format(dataset, method, met, self.num_RRDB)
            
        print(self.train_dir)
        print(self.save_path)
        # self.train_dir = './checkpoint/flat_CARS_B{}_CL_pls/'.format(self.num_repeat_RRDB)
        # self.train_dir = './checkpoint/CARS-B{}-CL/'.format(self.num_repeat_RRDB)
        # self.save_path = './test_p/test_result_flat_CARS_B{}_CL_pls/'.format(self.num_repeat_RRDB)
        # self.ckpt_dir = './checkpoint/CARS_B{}_CL'.format(self.num_repeat_RRDB)
        # self.test_path = './test_p/test_pls/test_img_pls/'
        # self.test_path = 'D:/Dataset/DFC2019/Train-Track1-RGB/test_input2/'
        
        self.channel = 3
        self.init_kernel = tf.keras.initializers.he_normal(seed=111)
        self.n_filter = 64
        self.inc_filter = 32
        ...
        elif dataset == 'pleiades':
            self.pathlist = open('./data/filelist_train_pleiades.txt', 'rt').read().splitlines()
            random.shuffle(self.pathlist)
            self.vallist = open('./data/filelist_val_pleiades.txt', 'rt').read().splitlines()
        elif dataset == 'DIV2K':
            self.pathlist = open('./data/filelist_train_DIV2K.txt', 'rt').read().splitlines()
            random.shuffle(self.pathlist)        
            self.vallist = open('./data/filelist_val_DIV2K.txt', 'rt').read().splitlines()
    ...
    def channel_attention(self, x, ratio, n_filter):
        with tf.variable_scope('CA'):
            _res = x
            x = tf.reduce_mean(x, axis = [1, 2], keepdims = True)
            x = tf.layers.conv2d(x, n_filter//ratio, 1, 1, padding = 'same', kernel_initializer=self.init_kernel, name='conv1')
            x = tf.nn.leaky_relu(x, alpha = 0.2, name = 'leakyReLU' )
            x = tf.layers.conv2d(x, n_filter, 1, 1, padding = 'same', kernel_initializer=self.init_kernel, name='conv2')
            x = tf.nn.sigmoid(x)
            # x = tf.nn.softmax(x)
    ...
    def _denseBlock(self, x, num=None):
        with tf.variable_scope('DenseBlock_sub{0}'.format(num)):
            ...
            if self.use_CA:
                if AM == 'CA':
                    x3 = self.channel_attention(x3, 16, self.n_filter)
                elif AM == 'CBAM':
                    x3 = cbam_block(x3, 'cbam_'+ str(num), ratio=8)
            # x4 = self.channel_attention(x3, 16, self.n_filter)
    ...
    def flat_denseBlock(self, x, num=None):
        with tf.variable_scope('DenseBlock_sub{0}'.format(num)):
            x1 = self._conv_RRDB(x, self.n_filter, 0)
            x2 = self._conv_RRDB(x1, self.n_filter, 1)
            x3 = self._conv_RRDB(x2, self.n_filter, 2, activate=False)
            if self.use_CA:
                if AM == 'CA':
                    x3 = self.channel_attention(x3, 16, self.n_filter)
                elif AM == 'CBAM':
    ...
    def _RRDB(self, x, num=None):
        ...
        with tf.variable_scope('RRDB_sub{0}'.format(num)):
            ...
            if self.use_densely:
                ...
                x_branch += self.flat_denseBlock(x_branch, 0)
                x_branch += self.flat_denseBlock(x_branch, 1)
                x_branch += self.flat_denseBlock(x_branch, 2)
            ...
            with tf.variable_scope('srmodel', reuse=reuse) as scope_sr:#prelu
                with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.relu, stride=1,
                    ...
                    with tf.variable_scope('first_conv'):
                        x = tf.layers.conv2d(rnn_input, self.n_filter, 3, 1, padding='same', kernel_initializer=self.init_kernel,
                                 name='conv')

                    with tf.variable_scope('RRDB'):
                        x_branch = tf.identity(x)

                        for i in range(self.num_RRDB):
                    ...
                    with tf.variable_scope('Upsampling'):
                        # x = tf.layers.conv2d(x, 48, 3, 1, padding='same', kernel_initializer=self.init_kernel,
                                 # name='upsampling_conv_1')
                        # x = modules.ps._PS(x, self.scale_factor, 3)
    ...
    def build_model(self):
        ...
        # mse = tf.reduce_mean(tf.abs(output - frame_gt_y) , axis=[0, 2, 3, 4])
        self.mse = mse
    ...
    def test(self, dataPath=None, scale_factor=4, num_frames=1):
        ...
        for pat in file:
            if file.index(pat)>=startid:
                ...
                if test_full:
                    ...
                    inp = [scipy.misc.imread(i)[400:600,400:600,:].astype(np.float32) / 255.0 for i in inList]
                ...
                for idx0 in range(len(inList)):
                    ...
                    # psnr_PECNN = sess.run(tf.image.psnr(inp_ref[0],imgs_hr_rgb[0,-1],max_val=1.0))
                    # psnr_bicubic = sess.run(tf.image.psnr(inp_ref[0],bic_ref,max_val=1.0))
                    # #psnr_PECNN = 10 * np.log10(1.0 / mse_acc)
                    # psnr_acc[cnt-1,0] = psnr_PECNN
                    # psnr_acc[cnt-1,1] = psnr_bicubic
                    # print('psnr_PECNN:',psnr_PECNN, 'psnr_bicubic', psnr_bicubic)
                    ...
                    # ssim_PECNN = sess.run( tf.image.ssim(inp_ref_,imgs_hr_rgb_,max_val=1.0))
                    # ssim_acc[cnt-1,0] = ssim_PECNN
                    # ssim_acc[cnt-1,1] = ssim_bicubic
                    # print('ssim_PECNN:',ssim_PECNN,'ssim_bicubic',ssim_bicubic)
                    # with open(os.path.join(self.train_dir, 'test_log.txt'), 'a+') as f:
                        # f.write('Img num {0:3d} -  PSNR_PECNN: {1:0.6f}, PSNR_bic:{2:0.6f},SSIM_PECNN: {3:0.6f},SSIM_bic:{4:0.6f}\n'.format(cnt, psnr_PECNN,
                                                                                        # psnr_bicubic, ssim_PECNN,ssim_bicubic))
        ...
        # print('psnr_PECNN_average:',psnr_avg[0],'psnr_bicubic_average:',psnr_avg[1])
        # print('ssim_PECNN_average:',ssim_avg[0],'ssim_bicubic_average:',ssim_avg[1])
        # with open(os.path.join(self.train_dir, 'test_log.txt'), 'a+') as f:
            # f.write('Average:- PSNR_PECNN_avg: {0:0.6f}, PSNR_bic_avg:{1:0.6f},SSIM_PECNN_avg: {2:0.6f},SSIM_bic_avg:{3:0.6f}\n'.format(psnr_avg[0],
